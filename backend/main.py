...
def _attach_feedback(alert: dict) -> dict:
    ...
    if feedback_json:
        try:
            ...
            logger.warning(
                f"Failed to parse feedback cache for {alert['alert_id']}: {e}"
            )
    return alert
...
@app.get("/")
async def root():
    return {"service": "TrueLift API", "version": "1.0.0", "status": "operational"}
            ...
            for alert in alerts:
                ...
                if "demo_queued_at" in alert and alert["demo_queued_at"]:
                    ...
                    if hasattr(dq, "isoformat"):
                        alert["demo_queued_at"] = (
                            dq.replace(tzinfo=timezone.utc).isoformat()
                            if dq.tzinfo is None
                            else dq.isoformat()
                        )
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            if alert_dict.get("recommended_action"):
                ...
                alert_dict["strategy"] = {
                    ...
                    else json.loads(recommended),
                    "alternatives": alternatives
                    if isinstance(alternatives, list)
                    else json.loads(alternatives),
...
async def internal_broadcast_feedback(payload: dict):
    ...
    """
    await manager.broadcast({"type": "feedback", "data": payload})
    return {"status": "ok"}
...
async def create_user_action(action: UserActionRequest):
    ...
    try:
        ...
        await manager.broadcast({"type": "action_taken", "data": feedback_data})
