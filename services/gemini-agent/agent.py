...
Provide actionable, data-driven recommendations."""
    ...
    def _store_strategy(self, alert_id: str, strategy: Dict):
        ...
        self.redis_client.setex(f"strategy:{alert_id}", 3600, json.dumps(strategy))
        ...
        for message in self.consumer:
            try:
                ...
                if (
                    cached_strategy
                    and last_generated
                    and (now_ts - int(last_generated) < self.rate_limit_window)
                ):
                ...
                logger.info(
                    f"✓ Strategy generated for {alert_id}: {strategy['primary_recommendation']['action']}"
                )
            except Exception as e:
                logger.error(
                    f"Error processing alert {alert_data.get('alert_id', 'unknown')}: {e}",
                    exc_info=True,
                )
