...
class SalesAggregator:
    def __init__(self, window_minutes: int = 1):
        ...
        self.sales_data = defaultdict(
            lambda: {"count": 0, "total_amount": 0, "events": []}
        )
    ...
    def should_aggregate(self) -> bool:
        return datetime.utcnow() >= self.window_start + timedelta(
...
class CannibalizationDetector:
    def __init__(self):
        ...
        # Remove SASL keys if credentials are missing (local dev)
        if not (
            self.kafka_config["sasl_plain_username"]
            and self.kafka_config["sasl_plain_password"]
        ):
            self.kafka_config.pop("sasl_mechanism", None)
            self.kafka_config.pop("sasl_plain_username", None)
            self.kafka_config.pop("sasl_plain_password", None)
            self.kafka_config.pop("security_protocol", None)
    ...
    def _detect_cannibalization(
        self, promo_id: str, actual_sales: int, aggregated_data: Dict
    ) -> bool:
    ...
    def _calculate_loss(
        self, promo_id: str, actual_sales: int, scale: float = 1.0
    ) -> Dict:
        promo_data = self._fetch_promotion_data(promo_id)
        if not promo_data:
            return {}
        ...
        predicted_sales_rounded = (
            max(1, int(round(predicted_sales_per_minute)))
            if predicted_sales_per_minute > 0
            else 0
        )
        sales_difference = predicted_sales_rounded - actual_sales
        loss_percentage = (
            (sales_difference / predicted_sales_per_minute * 100)
            if predicted_sales_per_minute > 0
            else 0
        )
    ...
    def _trigger_alert(
        self, promo_id: str, loss_data: Dict, burst_metadata: Dict = None
    ):
        promo_data = self._fetch_promotion_data(promo_id)
        if not promo_data:
            return
        ...
        if burst_metadata:
            ...
            if burst_metadata.get("burst_event_count") is not None:
                alert_data["burst_event_count"] = burst_metadata.get(
                    "burst_event_count"
                )
        ...
        try:
            ...
            logger.warning(
                f"🚨 CANNIBALIZATION DETECTED & PUBLISHED: {alert_data['product_name']} - "
            ...
            if alert_data.get("burst_id"):
                try:
                        ...
                        json.dumps(
                            {
                                ...
                                "burst_event_count": alert_data.get(
                                    "burst_event_count"
                                ),
                            }
                        ),
                    ...
                    logger.warning(
                        f"Failed to cache demo metadata for alert {alert_data['alert_id']}: {e}"
                    )
        except Exception as e:
                ...
                for promo_id, sales_info in aggregated_data.items():
                    ...
                    for burst_id, evs in burst_groups.items():
                        ...
                        is_cannibalized = self._detect_cannibalization(
                            promo_id,
                            actual_sales,
                            {**sales_info, "events": evs} if evs else sales_info,
                        )

                        if is_cannibalized:
                            loss_data = self._calculate_loss(
                                promo_id, actual_sales, scale=scale
                            )
                            if loss_data:
                                self._trigger_alert(
                                    promo_id, loss_data, burst_metadata=burst_meta
                                )
                        else:
                            logger.info(
                                f"✓ Normal sales for {promo_id}: {actual_sales} units"
                            )
